...
export const Element1 = () => {return <h2>HEELO WORLD - 1</h2>}
...
// {auth, name} = props (props 안에 있는 역할을 각각 auth, name으로 구조분해해서 할당한다는 의미)
// auth, name으로 바로 가져왔기 때문에 props.auth로 지정할 필요 X
export function Element4({auth, name}){
    ...
    if (auth === "ROLE_ADMIN"){
        ...
        return (
            <>
                <h2>유저님 환영합니다, NAME = {name} </h2>
            </>
        
        ) // 기본적으로 return을 이렇게 사용 (부모태그 필요하지만 한 줄이라 생략가능)
    }
}
